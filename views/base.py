...
class BaseMenu(object):
	...
	def getDic(self,k):
		if k in menuItemsDic.dic:
			return menuItemsDic.dic[k]
		return ""
		...
		if type(ref_id)==dict:
			for k,v in ref_id.items():
				if type(v) == str:
					...
					self._RegisterMenuCommand(menu_handle,k,self.getDic(k),None,index,v)
				if index>=0:index+=1
		elif type(ref_id)!=str and hasattr(ref_id,"__iter__"):
			for k in ref_id:
				self._RegisterMenuCommand(menu_handle,k,self.getDic(k),None,index)
				if index>=0:index+=1
		else:
			if not title:
				title=self.getDic(ref_id)
	...
	def RegisterCheckMenuCommand(self,menu_handle,ref_id,title="",index=-1):
		if type(ref_id)==dict:
			...
			for k in ref_id:
				self._RegisterCheckMenuCommand(menu_handle,k,self.getDic(k),index)
				if index>=0:index+=1
		else:
			if not title:
				title=self.getDic(ref_id)
