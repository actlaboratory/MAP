...
本ソフトウェアは、大学共同利用機関法人自然科学研究機構　生理学研究所の村田安永先生が開発されたソフトウェア「O2popper」(https://www.nips.ac.jp/~murata/o2popper/)を基に、機能とUIの見直し・アップデートを行って開発しました。
...
２．３　アップデート
発見された不具合の修正、新機能の追加、Gmailの仕様変更への対応などの目的で、定期的にバージョンアップを行います。
...
パスワードの入力、もしくはアカウントの選択の後、[このアプリは Google で確認されていません]という画面が表示された場合には、[詳細]ボタンを押します。
...
２．　複数のコンピュータからMAPを利用する場合には、いずれかのコンピュータでまとめてアカウント設定を行った後、「.credential」ディレクトリをコピーしてください。
　　　複数のMAPから同一のアカウントを追加してしまうと、前に追加したコンピュータではアカウントに接続できなくなります。
